...
def forward_general(self: 'BoundedModule', C=None, node:'Bound'=None, concretize=False,
    ...
    if (C is not None and isinstance(node, BoundLinear) and
            not node.is_input_perturbed(1) and not node.is_input_perturbed(2) and not node.input_shape[0] == 1):
    ...
    if C is not None and not C_merged and not node.input_shape[0] == 1:
