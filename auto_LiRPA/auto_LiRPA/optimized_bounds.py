...
def _update_best_ret(full_ret_bound, best_ret_bound, full_ret, best_ret,
    ...
    if improved_idx is not None:
        ...
        if not deterministic:
            best_ret_bound[0][0][improved_idx] = compare(
                full_ret_bound[0][0][improved_idx], best_ret_bound[0][0][improved_idx])
        else:
            best_ret_bound[improved_idx] = full_ret_bound[improved_idx]
        if full_ret[idx] is not None:
            if not deterministic:
                best_ret[idx][0][0][improved_idx] = compare(
                    full_ret[idx][0][0][improved_idx],
                    best_ret[idx][0][0][improved_idx])
            else:
